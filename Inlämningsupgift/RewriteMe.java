...
//    return  questions.stream().filter(s -> s.getQuestionString()
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory() {
        return questions.stream().collect(groupingBy(Question::getCategory,
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c) {
        return questions.stream().filter(category -> category.getCategory().equals(c))
